...
function Homepage({ passWeatherData, weatherImages }) {
  ...
  const searchPressed = () => {
    fetch(
      `${weather_api.base}weather?q=${search}&units=metric&APPID=${weather_api.key}`
    )
      .then((res) => res.json())
      .then((result1) => {
        setWeather(result1);
    ...
    fetch(
      `${forecast_api.base}forecast?q=${search}&units=metric&cnt=${7}&appid=${
        forecast_api.key
      }`
    )
      .then((res) => res.json())
  ...
  return (
    <div
      className={
        typeof weather.main != "undefined" ? renderBackgroundImage() : "screen"
      }
    >
      <div className="main">
        ...
        <header>
          {typeof weather.main != "undefined" ? <Navbar /> : ""}
        ...
        {typeof weather.main == "undefined" ? (
          <>
            <img
              className="images"
              src="https://i1.sndcdn.com/avatars-000133198400-ercv7n-t500x500.jpg"
              alt="default image"
            />
          </>
        ) : (
        ...
        {typeof weather.main != "undefined" ? <div></div> : ""}
      </div>
    </div>
  );
}
